...
//	public ArrayList<String> onlineStatusClients ;//= new ArrayList<String>();
	...
	public void start() {
		...
		try 
		{
			...
			while(keepRunning) 
			{
			    ...
			    // add groupsMapFinal
			    serRes.setGroupUpdate(groupsMapFinal);
			    // add updated client List
			    serRes.setClientUpdate(getUpdatedOnlineClientList());
	...
	private synchronized void broadcast(ChatMessage chatMsg) {
		...
		String messageLf = chatMsg.getClient() + " : " + chatMsg.getMessage() + "\n";
	...
	private synchronized void unicast(ChatMessage chatMsg) {
		...
		String FromClientName = chatMsg.getClient();
		...
		for(int i = liveClientList.size(); --i >= 0;) {
			...
			if( (ct.getUsername().equals(chatMsg.getTarget())) || (ct.getUsername().equals(chatMsg.getClient())) ){
	...
	// to unicast message to respective client
		private synchronized void multicast(ChatMessage chatMsg) {
			...
			for(int i = liveClientList.size(); --i >= 0;) {
				ClientThread ct = liveClientList.get(i);
				// looking for the unicast target client and the from client (who send msg)
				if( (ct.getMemberGroupList().contains(chatMsg.getTarget())) || (ct.getOwnGroupList().contains(chatMsg.getTarget()))   )
				{// || (ct.getUsername().equals(chatMsg.getClientName())) ){
	...
	public void createGroup(ChatMessage chtMsg){
		...
		display(chtMsg.getClient());
		display(chtMsg.getCastType());
		display(chtMsg.getGrp());
		display(chtMsg.getTarget());
		// verify the client is live or not
		ClientThread presentClient = getClientThread(chtMsg.getClient());
		...
		if(presentClient.getOwnGroupList().contains(chtMsg.getGrp())){
		...
		presentClient.getOwnGroupList().add(chtMsg.getGrp()); // updating with client
            HashSet groupMembersSet = new HashSet<String>();
            groupMembersSet.add(chtMsg.getClient());
		    groupsMapFinal.put(chtMsg.getGrp(),groupMembersSet); // updating with final members map
	...
	// Join to an existed group
		public void joinGroup(ChatMessage chtMsg){
			...
			System.out.println(chtMsg.getClient());
			System.out.println(chtMsg.getCastType());
			System.out.println(chtMsg.getGrp());
			System.out.println(chtMsg.getTarget());
			...
			// verify the group exists or not
			if(presentClient.getMemberGroupList().contains(chtMsg.getGrp())){
				System.out.println("User is already joined to this Group");return;
			}
			
			// add group to the clientThread avtive clients list
			   presentClient.getMemberGroupList().add(chtMsg.getGrp()); // updating with client
	            HashSet groupMembersSet =groupsMapFinal.get(chtMsg.getGrp());
			...
			System.out.println(chtMsg.getTarget());
			        ...
			        // verify the client is live or not
						...
						if(presentClient.getMemberGroupList().contains(chtMsg.getGrp())){
							...
							presentClient.getMemberGroupList().remove(chtMsg.getGrp());
							
							// leaving from finalmap group as member
							HashSet groupMembersSet = groupsMapFinal.get(chtMsg.getGrp());
							
							 if(groupMembersSet == null)
							 { System.out.println("You are not a member of this group to leave");return;}
							boolean res = groupMembersSet.remove(chtMsg.getClient());
						...
						else if(presentClient.getOwnGroupList().contains(chtMsg.getGrp())){
							// leaving own group
							
							// remove from client grouplist
							presentClient.getOwnGroupList().remove(chtMsg.getGrp());
							// remove from finalmap group
							groupsMapFinal.remove(chtMsg.getGrp());
		...
		public void run() {
			...
			while(keepRunning) {
				...
				case ChatMessage.MESSAGE:
					String castType = cm.getCastType();
					String targetVal = cm.getTarget();
					     switch(castType){
					     // for uni casting
				...
				case ChatMessage.OFFLINE:
					...
					if (cm.getClient() != null){
						
						updateClientStatus(cm.getClient(), false);
				...
				case ChatMessage.ONLINE:
					...
					if (cm.getClient() != null){
						...
						// update all clients.
						updateClientStatus(cm.getClient(), true);
						ServerResponse serRes = new ServerResponse();
					    serRes.setRespType("UGT&UCL"); // UGT for update Group Tree  & UCL for update clientList
					    serRes.setMessage(null);
					    // add groupsMapFinal
