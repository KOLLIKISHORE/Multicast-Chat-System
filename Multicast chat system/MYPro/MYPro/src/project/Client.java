...
public class Client  {
	...
	public JTree UpdateGroupList(HashMap<String, HashSet<String>> currentGroupList){
		...
		JTree updatedmulticastTree = new JTree(rootNode);
	...
	class ListenFromServer extends Thread {
		...
		public void run() {
			while(true) {
				...
				System.out.println("Server Response saying group size is = "+sresp.getGroupUpdate().size());
						
						if(sresp.getRespType().equalsIgnoreCase("UGT")){ // selected update Group tree
							
							
							JTree newJtree = UpdateGroupList(sresp.getGroupUpdate());
						   ...
						   // update list
							...
							DefaultListModel<String> updatedClientListModel  = updatedOnlineClientsList (sresp.getClientUpdate());
							cg.uni_cst_Lst.setModel(updatedClientListModel);
					...
					display("server connection is closed. please check once " + e);
					if(cg != null) 
						cg.connectionFailed();
					break;
				}
